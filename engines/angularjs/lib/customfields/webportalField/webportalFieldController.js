define(['angularAMD', 'utilService', 'fieldService', 'gridService'], function (angularAMD) {
    'use strict';

    angularAMD.controller('webPortalFieldController', function ($scope, $injector, $http, $location, $controller, httpService, overlay, Notification, $filter,
        $templateCache, utilService, fieldService, gridService) {
        var self = this;

        var fieldConfigUrl = 'engines/angularjs/lib/customfields/webportalField/webportalField.json';

        console.log("fieldConfigUrl", fieldConfigUrl);
        var loadingFieldConfiguration = function () {
            //  $http.get(fieldConfigUrl).then(function (remoteData) {
            $http.get(fieldConfigUrl).success(function (remoteData) {

                 $scope.sections = remoteData.sections;
        ...
        $scope.loadFieldDataFromGlobalDataServices = function (globalDataServices) {
            for (var i = 0; i < globalDataServices.length; i++) {
                utilService.loadFieldDataFromSingleService(globalDataServices[i], function (targetFieldDataMap, originalServiceData) {
                    console.log("targetFieldDataMap", targetFieldDataMap);
                    utilService.loadFormFieldsData($scope.sections, targetFieldDataMap);
                });
            }
        ...
        $scope.populateSectionModel = function (sectionDefs, sourceModel, targetModel) {
            ...
            for (var i = 0; i < sections.length; i++) {
                ...
                for (var j = 0; j < rowItemCount; j++) {
                    var rowFields = rowDivisions[j]["fields"];
                    for (var k = 0; k < rowFields.length; k++) {
                        var field = rowFields[k];
                        console.log("field==", field)
                        if (field.defaultValue) {
                            $scope.targetModel[field.modelKey] = field.defaultValue;
                        }
                        else if (field.defaultValueByKey) {
                            $scope.targetModel[field.modelKey] = utilService.getValueFromModelByKey(field.defaultValueByKey, sourceModel, null, $scope.parentForm);
                        }
        ...
        $scope.field.getModel = function () {
            ...
            for (var i = 0; i < modelKeyMap.length; i++) {
                ...
                model[modelKey] = utilService.getValueFromModelByKey(modelKey, fieldViewModel, null, $scope.parentForm);
            }
            return model;
        };
                ...
                if (additionalKeyParams && source) {
                    ...
                    angular.forEach(additionalKeyParams, function (modelKey, paramName) {
                        if (modelKey) {
                            ...
                            var sourceDataModelValue = utilService.getValueFromModelByKey(modelKey, source, $scope.parentSection.modelKey, $scope.parentForm);
